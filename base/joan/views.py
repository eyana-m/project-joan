...
def get_current_sprint(project):
    try:
        return Sprint.objects.filter(release__project__id__exact=project).filter(status__exact='AC').get()
    except:
...
class ProjectView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        current_sprint = Sprint.objects.filter(release__project__id__exact=self.kwargs['pk']).filter(status__exact='AC').get()
        ...
        context['features_done_count'] = Feature.objects.filter(release__project__id__exact=self.kwargs['pk']).filter(status__exact='DO').count()
        context['features_done_percentage'] = percentage(context['features_done_count'],context['features_count'] )

        uc_done_only = Feature.objects.filter(release__project__id__exact=self.kwargs['pk']).filter(status__exact='DU').count()
        ...
        context['features_for_fv_count'] = Feature.objects.filter(release__project__id__exact=self.kwargs['pk']).filter(status__exact='FV').count()
...
class ReleaseView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['done_requirement_list'] = Requirement.objects.distinct().filter(feature__release__id__exact=self.kwargs['pk']).filter(feature__status__exact='DO').exclude(feature__status__in=('NW', 'DU', 'FV'))

        context['done_feature_list'] = Feature.objects.filter(release__id__exact=self.kwargs['pk']).filter(status__exact='DO')

        context['done_feature_list'] = Feature.objects.filter(release__id__exact=self.kwargs['pk']).filter(status__exact='DO')

        context['done_sprint_list'] = Sprint.objects.filter(release__id__exact=self.kwargs['pk']).filter(status__exact='DO')
...
class ProjectRequirementsView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Select all requirements where all features are done
        context['done_requirement_list'] = Requirement.objects.distinct().filter(project__id__exact=self.kwargs['pk']).filter(feature__status__exact='DO').exclude(feature__status__in=('NW', 'DU', 'FV'))
        context['ongoing_requirements_count'] = Requirement.objects.distinct().filter(project__id__exact=self.kwargs['pk']).filter(feature__status__in=('NW', 'DU', 'FV')).count() + Requirement.objects.distinct().filter(project__id__exact=self.kwargs['pk']).filter(feature__status__isnull=True).count()
...
class ProjectFeaturesView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['done_feature_list'] = Feature.objects.distinct().filter(release__project__id__exact=self.kwargs['pk']).filter(status__exact='DO')
        context['ongoing_features_count'] = Feature.objects.distinct().filter(release__project__id__exact=self.kwargs['pk']).filter(status__in=('NW', 'DU', 'FV')).count() + Feature.objects.distinct().filter(release__project__id__exact=self.kwargs['pk']).filter(status__isnull=True).count()
...
class SprintView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['all_features'] = Feature.objects.distinct().filter(ticket__sprint__id__exact=self.kwargs['pk'])
        context['done_tickets'] = Ticket.objects.filter(sprint_id__exact=self.kwargs['pk']).filter(status__exact='DO')
